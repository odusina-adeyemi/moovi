...
const initialState = {
  ...
  const localStorageMiddleware = (store) => (next) => (action) => {
    ...
    if (action.type === actionLoad.SELECTED_GENRES_UPDATE || action.type === actionLoad.MOVIE_DATA_SUCCESS) {
      localStorage.setItem('selectedGenres', JSON.stringify(store.getState().selectedGenres));
      localStorage.setItem('movieData', JSON.stringify(store.getState().movieData));
    }
    return result;
  };
